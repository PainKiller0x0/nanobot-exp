...
function applyTheme(mode){const dark=mode==='dark';const value=dark?'dark':'light';root.setAttribute('data-theme',value);root.classList.toggle('dark',dark);if(document.body){document.body.setAttribute('data-theme',value);document.body.classList.toggle('dark',dark)}['notifyTheme','sidecarShellTheme','dashboardTheme'].forEach(k=>localStorage[k]=value)}
applyTheme((localStorage.sidecarShellTheme||localStorage.dashboardTheme||localStorage.notifyTheme)==='dark'?'dark':'light');let JOBS=[];
