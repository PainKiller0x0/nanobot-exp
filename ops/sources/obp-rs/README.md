...
- 模型列表：`/v1/models`
- 管理接口：`/admin/channels`、`/admin/router`、`/admin/stats`、`/admin/deepseek/balance`
- 路由组：默认、高级、应急、备用
- 来源路由：按 `default-nanobot`、`guangzhou-nanobot` 等来源指定不同路由组
- 成本账本：区分付费、免费、总消耗
- DeepSeek 余额哨兵：通过官方 `/user/balance` 保存余额快照，方便估算 OBP 外部的同 key 消耗
...
- `data/deepseek_balance.json`：DeepSeek 余额快照，不要提交到 Git。
