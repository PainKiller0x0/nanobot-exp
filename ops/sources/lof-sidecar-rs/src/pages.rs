...
<div class="wrap">
  ...
  <section id="sec-tasks" class="section grid"><article class="card full"><h2>任务追踪</h2><p class="sub">第一版展示配置、最近一次运行、是否发送、未发送原因和一键重跑。Notify 当前只保存最近一次状态，后续可以再加 7 次历史。</p><div class="filterbar"><button class="secondary" onclick="taskFilter='all';renderTasks()">全部</button><button class="secondary" onclick="taskFilter='bad';renderTasks()">异常</button><button class="secondary" onclick="taskFilter='sent';renderTasks()">已发送</button><button class="secondary" onclick="taskFilter='silent';renderTasks()">静默</button><input id="taskKw" class="input" placeholder="筛选任务名/id" oninput="renderTasks()"></div><div style="overflow:auto"><table><thead><tr><th>任务</th><th>规则</th><th>最近状态</th><th>下次运行</th><th>解释</th><th>操作</th></tr></thead><tbody id="taskRows"></tbody></table></div></article></section>
  <section id="sec-model" class="section grid"><article class="card full"><h2>OBP 模型路由解释</h2><div id="modelSummary"></div></article><article class="card wide"><h2>最近请求</h2><div style="overflow:auto"><table><thead><tr><th>时间</th><th>来源</th><th>路由</th><th>模型</th><th>原因</th><th>成本</th></tr></thead><tbody id="modelRows"></tbody></table></div></article><article class="card"><h2>升级 Pro 的原因</h2><div class="list" id="proReasons"></div></article></section>
</div>
<script>
const root=document.documentElement;
...
function renderModel(){const d=DATA.model;if(!d)return;const paid=d.stats?.paid?.total||{}, free=d.stats?.free?.total||{};document.getElementById('modelSummary').innerHTML=`<div class="stats"><div class="stat"><div class="k">付费请求</div><div class="v">${paid.requests||0}</div><div class="mini muted">￥${Number(paid.cost_cny||0).toFixed(4)}</div></div><div class="stat"><div class="k">免费请求</div><div class="v ok">${free.requests||0}</div></div><div class="stat"><div class="k">Pro 请求</div><div class="v warn">${d.pro_count||0}</div></div><div class="stat"><div class="k">最近记录</div><div class="v">${(d.recent||[]).length}</div></div></div>`;document.getElementById('modelRows').innerHTML=(d.recent||[]).map(x=>`<tr><td>${esc(x.time||'-')}</td><td>${esc(x.source||'-')}</td><td>${esc(x.route||'-')}</td><td>${esc(x.model||'-')}<div class="mini muted">请求：${esc(x.requested_model||'-')}</div></td><td>${esc(x.route_reason||'-')}</td><td>￥${Number(x.cost_cny||0).toFixed(5)}<div class="mini muted">${x.prompt_tokens||0}/${x.completion_tokens||0} tokens</div></td></tr>`).join('')||'<tr><td colspan="6" class="empty">暂无模型调用记录。</td></tr>';document.getElementById('proReasons').innerHTML=Object.entries(d.pro_reasons||{}).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div class="item"><div class="row"><b>${esc(k)}</b><span class="pill warn">${v} 次</span></div></div>`).join('')||'<div class="empty">最近没有升级 Pro。</div>'}
