...
function renderSystem(){const m=state.system?.memory||{};const pressure=loadFeeling(state.system?.loadavg||{},state.system?.cpu||{});const pct=clamp(num(m.used_pct)||0,0,100);const memCls=pct>75?'warnText':'good';document.getElementById('systemMetrics').innerHTML=`<div class="systemFeel"><div class="systemHero"><div><div class="k">当前体感</div><div class="feelMain ${pct>75?'warnText':'good'}">${pct>75?'偏紧':'轻快'}</div></div><div class="feelMeta">以内存和 CPU 压力估算<br>只保留日常有用指标</div></div><div><div class="row mini"><span class="muted">内存占用</span><b class="${memCls}">${m.used_pct??'-'}%</b></div><div class="gauge"><i style="width:${pct}%"></i></div></div><div class="systemFacts"><div class="fact"><div class="k">内存</div><div class="v ${memCls}">${m.used_mb??'-'} MB</div><div class="note">总量 ${m.total_mb??'-'} MB，可用 ${m.available_mb??'-'} MB</div></div><div class="fact"><div class="k">CPU 压力</div><div class="v ${pressure.cls}">${pressure.label}</div><div class="note">${esc(pressure.note)}；不是网络延迟</div></div></div></div>`}
