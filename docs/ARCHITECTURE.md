...
- Direct replies：无 LLM 快捷回复保留在 agent 层；系统内存直查放在 `nanobot/agent/system_reply.py`，能力注册读取放在 `nanobot/agent/capability_registry.py`，dashboard 聚合读取放在 `nanobot/agent/capability_snapshot.py`，能力/收件箱/记忆分别在 `capability_reply.py`、`inbox_reply.py`、`memory_reply.py`，`direct_reply.py` 只做路由和兜底短句判断。
