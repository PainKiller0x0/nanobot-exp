...
- QQ channel：`ops/config/overrides/qq.py` 是线上 QQ 通道胶水覆盖实现，容器启动时由 `/root/.nanobot/overrides/apply_overrides.py` 覆盖到 `/app/nanobot/channels/qq.py`。QQ 文件应尽量保留上游 botpy 适配结构；nanobot-exp 自定义策略放在 `nanobot/exp/qq/`：
  - `streaming.py`：流式策略。
  - `stream_runtime.py`：流式状态机和 delta flush。
  - `fast_paths.py`：本地快捷指令匹配。
  - `article_requests.py`：微信/鸭哥文章请求解析。
  - `article_runtime.py`：RSS/鸭哥文章 Rust API 优先、旧脚本 fallback 的运行时 Adapter。
  - `local_commands.py`：本地 skill 命令 runner。
  - `signatures.py`：签名/ACK 解析。
  - `signed_delivery.py`：签名 payload 防篡改、自修复和投递确认。
  - `rss_sidecar.py`：QQ 到 RSS Rust sidecar 的 HTTP Adapter。
