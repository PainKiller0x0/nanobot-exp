...
# Keep runtime channel installs isolated from application source. Optional
# channels may add their own manifest-declared dependencies after startup.
...
RUN NANOBOT_FORCE_WEBUI_BUILD=1 uv pip install --python "$VIRTUAL_ENV/bin/python" --no-cache .
...
# The entrypoint drops privileges after preparing a mounted data directory.
USER root
